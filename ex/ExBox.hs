...
boxPlusUnboxed a = unbox . boxPlus a

unbox :: Box a -> a
unbox (Box x) = x

applyBoxed :: Box (a -> b) -> Box a -> Box b
applyBoxed (Box f) (Box x) = Box result where result = f x

applyWithinBox :: Box (a -> b) -> a -> Box b
applyWithinBox f x = applyBoxed f $ Box x

composeBoxedFuns :: Box (b -> c) -> Box (a -> b) -> Box (a -> c)
composeBoxedFuns (Box f1) (Box f2) = Box (f1 . f2)

headOfBoxed :: Box [a] -> a
headOfBoxed (Box []     ) = error "empty list"
headOfBoxed (Box (h : _)) = h
